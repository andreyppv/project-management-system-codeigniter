...
function getMessages($db, $projectID){
  $prepared = $db->prepare("SELECT * FROM messages WHERE messages_project_id = ? AND messages_by = 'team' ORDER BY messages_date DESC");
  $prepared->execute(array($projectID));
  return $prepared->fetchAll();
}
                              ...
                              <script>
                                $(document).ready(function(){
                                  $(".addreplyform<?php echo htmlentities($message['messages_id']); ?>").submit(function(e){
                                    $.post( 
                                      "http://pms.isodeveloper.com/__freshbooksapi/_createreply.php?team", 
                                      { 

                                        messages_replies_text: $("#message_reply_11_team_<?php echo htmlentities($message['messages_id']); ?>").val(), 
                                ...
                                <form class="addreplyform<?php echo htmlentities($message['messages_id']); ?>" action="" method="post">
                                  <div class="form-group form-group-standard">
                                    <div class="col-lg-12">
                                      <input type="text" name="messages_replies_text" id="message_reply_11_team_<?php echo htmlentities($message['messages_id']); ?>"/>
                                    </div>
                                  </div>
                                  <div class="form-group form-group-standard">
                                    <div class="col-lg-12">
