...
// *********************************************
//NEXTLOOP changed from clsTinyButStrong to Tbs for code ignitor
class Tbs
{
    ...
    /** NEXTLOOP: Codeigniter variable **/
    public $ci;

    function __construct($Options = null, $VarPrefix = '', $FctPrefix = '')
    {

        /** NEXTLOOP: GET/SET CODEIGNITER INSTANCE **/
    ...
    function meth_Locator_Replace(&$Txt, &$Loc, &$Value, $SubStart)
    {
            ...
            for ($i = $SubStart; $i < $Loc->SubNbr; $i++) {
                ...
                if (is_array($Value)) {
                    ...
                    if (method_exists($Value, $x)) {
                        if ($this->MethodsAllowed or !in_array(strtok($Loc->FullName, '.'), array(
                            'onload',
                            'onshow',
                            'var'))) {
            ...
            foreach ($Loc->OpeAct as $i => $ope) {
                switch ($ope) {
                    ...
                    case 1:
                        ...
                        break;
                    case 2:
                        if (strlen('' . $CurrVal) > $Loc->OpePrm[$i]) $this->f_Xml_Max($CurrVal, $Loc->OpePrm[$i], $Loc->OpeEnd);
                        break;
                    case 3:
                        if (strlen('' . $CurrVal) > $Loc->OpePrm[$i]) $CurrVal = (($Loc->OpeUtf8) ? mb_substr('' . $CurrVal, 0, $Loc->OpePrm[$i], 'UTF-8') : substr('' . $CurrVal, 0, $Loc->OpePrm[$i])) . $Loc->OpeEnd;
                        break;
                    case 5:
                    ...
                    case 11:
                        ...
                        break;
                    case 12:
                        if (@(string )$CurrVal === $Loc->OpePrm[$i]) $CurrVal = '';
                        break;
                    case 13:
                    ...
                    case 14:
                        ...
                        break;
                    case 15:
                        $CurrVal = ($Loc->OpeUtf8) ? mb_convert_case($CurrVal, MB_CASE_UPPER, 'UTF-8') : strtoupper($CurrVal);
                        break;
                    case 16:
                    ...
                    case 17:
                        ...
                        break;
                    case 18:
                        $CurrVal = ($Loc->OpeUtf8) ? mb_convert_case($CurrVal, MB_CASE_TITLE, 'UTF-8') : ucwords($CurrVal);
                        break;
                }
            }
        }
        ...
        if ($CurrVal === '') {
            ...
            switch ($Loc->MagnetId) {
                ...
                case - 1:
                    $CurrVal = '&nbsp;';
                    break; // Enables to avoid null cells in HTML tables
                case - 2:
    ...
    function meth_Merge_AutoSpe(&$Txt, &$Loc)
    {
        ...
        if (isset($Loc->SubLst[1])) {
            switch ($Loc->SubLst[1]) {
                ...
                case 'template_date':
                    $x = '';
                    if ($this->f_Misc_GetFile($x, $this->_LastFile, '', array(), false)) $x = $x['mtime'];
                    break;
                case 'template_path':
    ...
    static function f_Loc_Enlarge_Find($Txt, $Tag, $Fct, $Pos, $Forward, $LevelStop)
    {
        if ($Fct === false) {
            ...
            $p = call_user_func_array($Fct, array(
                $Tag,
                $Txt,
                $Pos,
                $Forward,
    ...
    static function f_Xml_AttFind(&$Txt, &$Loc, $Move = false, $AttDelim = false)
    {
        ...
        if (isset($LocO->PrmLst[$AttLC])) {
            ...
            $Loc->AttBeg = $p[0];
            $p[3]--;
            while ($Txt[$p[3]] === ' ') $p[3]--; // external end of the attribute, may has an extra spaces
            $Loc->AttEnd = $p[3];
            $Loc->AttDelimCnt = $p[5];
            $Loc->AttDelimChr = $p[4];
    ...
    static function f_Xml_FindTagStart(&$Txt, $Tag, $Opening, $PosBeg, $Forward, $Case = true)
    {
        ...
        */
        ...
        if ($Case) {
            do {
                if ($Forward) $p = strpos($Txt, $x, $p + 1);
                else  $p = strrpos(substr($Txt, 0, $p + 1), $x);
                if ($p === false) return false;
            ...
            do {
                if ($Forward) $p = stripos($Txt, $x, $p + 1);
                else  $p = strripos(substr($Txt, 0, $p + 1), $x);
                if ($p === false) return false;
                $z = substr($Txt, $p + $xl, 1);
            } while (($z !== ' ') && ($z !== "\r") && ($z !== "\n") && ($z !== '>') && ($z !== '/') && ($Tag !== '/') && ($Tag !== ''));
        }
