...
class Tasks extends MY_Controller
{
    ...
    function __tasksView()
    {
        ...
        $this->data['blocks']['tasks'] = $this->task_model
            ->select('t1.*, t2.*, t3.team_profile_full_name asssigned_by_name, estimatedhours*IFNULL(bcat_rate, 125) estimatedcost, 
                IFNULL(bcat_name, "N/A") bcat_name, IFNULL(bcat_rate, 125) bcat_rate, IFNULL(bcat_label_class, "label-default") bcat_label_class', FALSE)
            ->join_assigned_to()
            ->join_assigned_by()
        ...
        $select_sum = 'SUM(estimatedhours*IFNULL(bcat_rate, 125)) estimated_total, 
            SUM(hourslogged*IFNULL(bcat_rate, 125)) worked_total,
            SUM(hourslogged*t2.hourlyrate) development_total';     
	...
	function __editDescription()
    {
        ...
        redirect('/admin/tasksexpanded/' . $task_projectid . '/'.$task_id.'');
