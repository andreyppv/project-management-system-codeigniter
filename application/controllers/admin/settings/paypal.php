...
/**
 * class for perfoming all Paypal Settings related functions
 *
 * @author   Nextloop.net
 * @access   public
...
class Paypal extends MY_Controller
{
    ...
    public function __construct()
    {
        ...
        //template file
        $this->data['template_file'] = PATHS_ADMIN_THEME . 'settings.paypal.html';
    ...
    function index()
    {
        ...
        //default page title
        $this->data['vars']['main_title'] = $this->data['lang']['lang_settings_paypal'];
    ...
    function __viewSettings()
    {
        ...
        if ($next) {
            ...
            $this->data['reg_fields'][] = 'paypal';
            $this->data['fields']['paypal'] = $this->settings_paypal_model->getSettings();
            $this->data['debug'][] = $this->settings_paypal_model->debug_data;

            if ($this->data['fields']['paypal']) {

                //show form
                $this->data['visible']['wi_paypal_settings'] = 1;
    ...
    function __editSettings()
    {
        ...
        if (!isset($_POST['submit'])) {
            redirect('/admin/settings/paypal/view');
        }
        ...
        //edit settings
        if ($next) {
            $result = $this->settings_paypal_model->editSettings();
            $this->data['debug'][] = $this->settings_paypal_model->debug_data;
            if ($result) {
                //show success
        ...
        if ($next) {
            //new status
            $new_status = ($this->input->post('paypal_active') == 'yes') ? 'enabled' : 'disabled';
            //update payment methods table also
            $result = $this->settings_payment_methods_model->updateStatus('paypal', $new_status);
    ...
    function __flmFormValidation($form = '')
    {
        ...
        if ($form == 'edit_settings') {
            ...
            $fields = array(
                'paypal_active' => $this->data['lang']['lang_active'],
                'paypal_email_address' => $this->data['lang']['lang_email_address'],
                'paypal_currency' => $this->data['lang']['lang_currency']);
