...
/**
 * class for perfoming all cash Settings related functions
 *
 * @author   Nextloop.net
 * @access   public
...
class Cash extends MY_Controller
{
    ...
    public function __construct()
    {
        ...
        //template file
        $this->data['template_file'] = PATHS_ADMIN_THEME . 'settings.cash.html';
    ...
    function index()
    {
        ...
        //default page title
        $this->data['vars']['main_title'] = $this->data['lang']['lang_settings_cash'];
    ...
    function __viewSettings()
    {
        ...
        if ($next) {
            ...
            $this->data['reg_fields'][] = 'cash';
            $this->data['fields']['cash'] = $this->settings_cash_model->getSettings();
            $this->data['debug'][] = $this->settings_cash_model->debug_data;

            if ($this->data['fields']['cash']) {

                //show form
                $this->data['visible']['wi_cash_settings'] = 1;
    ...
    function __editSettings()
    {
        ...
        if (!isset($_POST['submit'])) {
            redirect('/admin/settings/cash/view');
        }
        ...
        //edit settings
        if ($next) {
            $result = $this->settings_cash_model->editSettings();
            $this->data['debug'][] = $this->lang_settings_cash->debug_data;
            if ($result) {
                //show success
        ...
        if ($next) {
            //new status
            $new_status = ($this->input->post('cash_active') == 'yes') ? 'enabled' : 'disabled';
            //update payment methods table also
            $result = $this->settings_payment_methods_model->updateStatus('cash', $new_status);
    ...
    function __flmFormValidation($form = '')
    {
        ...
        if ($form == 'edit_settings') {
            ...
            $fields = array(
                'cash_active' => $this->data['lang']['lang_active'],
                'settings_cash_details' => $this->data['lang']['lang_email_address']);
