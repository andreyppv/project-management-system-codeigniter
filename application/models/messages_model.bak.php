...
class Messages_model extends Super_Model
{
    ...
    public function addMessageClient()
    {
        //profiling::
        $this->debug_methods_trail[] = __function__;
		...
		$query = $this->db->query("SELECT client_users_full_name FROM client_users WHERE client_users_id = ".$messages_by_id."");
        $result = $query->result_array()[0];
        $usrname = $result['client_users_full_name'];
		...
		$this->db->where('project_members.project_members_project_id',str_replace("'", "", $messages_project_id));
		...
		foreach($this->db->get()->result() as $row)
		{
            ...
            $email_vars['messages_text']  = $this->input->post('messages_text');
        ...
        //mod by Tomasz
        /*$text=addslashes($this->data['vars']['my_name'].' added new message in '.str_replace("'", "", $name->projects_title).'&#39;s <a href="'.site_url().'/admin/messages/'.str_replace("'", "", $messages_project_id).'/view">client chat</a>');*/
        $messages_project_id = str_replace("'", "", $messages_project_id);
        $myname = str_replace("'", "", $myname);
        ...
        $text = mysql_real_escape_string(sprintf($text_template, 
            $this->data['vars']['my_name'], 
            $name->projects_title,
            site_url("client/messages/$messages_project_id/view")         
        ));
        //end by Tomasz
    ...
    function addMessage()
    {
		...
		$query = $this->db->query("SELECT team_profile_full_name FROM team_profile WHERE team_profile_id = ".$messages_by_id."");
        $result = $query->result_array()[0];
