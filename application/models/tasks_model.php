...
class Tasks_model extends Super_Model
{
    ...
    /*
    * get list of time doc tasks
    */
    public function listTasksByClient($clients_id)
    {
        //profiling::
        $this->debug_methods_trail[] = __function__;
        ...
        //escape data
        $client_id = intval($clients_id);

        //----------sql & benchmarking start----------
        $this->benchmark->mark('code_start');

        //_____SQL QUERY_______
        $this->db->select('tasks.*, projects.*')
                 ->from('projects')
                 ->join('tasks', 'tasks.tasks_project_id = projects.projects_id')
                 ->where('projects.projects_clients_id', $clients_id);
    ...
    public function listTasks($offset = 0, $type = 'search', $project_id = '', $status = 'pending')
    {
        $status = "all-open";
        //profiling::
        $this->debug_methods_trail[] = __function__;
        ...
        //if no value project_id, return false
        ...
        //_____SQL QUERY_______
        $query = $this->db->query("SELECT tasks.*, team_profile.*, t3.team_profile_full_name asssigned_by_name
                                             FROM tasks
                                             LEFT OUTER JOIN team_profile ON tasks.tasks_assigned_to_id = team_profile.team_profile_id
                                             LEFT OUTER JOIN team_profile t3 ON tasks.tasks_created_by_id = t3.team_profile_id
        ...
        if ($transaction_result === false) {
            ...
            log_message('error', '[FILE: ' . __file__ . ']  [public function: ' . __function__ . ']  [LINE: ' . __line__ . "]  [MESSAGE: Database Error -  $db_error]");
    ...
    public function addTask()
    {
        ...
        foreach ($_POST as $key => $value) {
            if( in_array($key, array(
                    'tasks_assigned_to_id', 'tasks_client_id', 'tasks_created_by_id',
                    'tasks_milestones_id', 'tasks_project_id', 'billing_category'
                )) )
            {
                $$key = intval($value);
                continue;
        ...
        $this->db->where('projects_id', $tasks_project_id);
        $name = $this->db->get()->row_array()['projects_title'];
        //mod by Tomasz
    ...
    public function deleteTask($id = '', $delete_by = '')
    {
        //profiling::
        $this->debug_methods_trail[] = __function__;
        ...
        if (! is_numeric($id)) {
            ...
            log_message('error', '[FILE: ' . __file__ . ']  [public function: ' . __function__ . ']  [LINE: ' . __line__ . "]  [MESSAGE: deleting task(s) failed (tasks_id: $id is invalid)]");
            return false;
        }
        ...
        if (! in_array($delete_by, $valid_delete_by)) {
            ...
            //ajax-log error to file
            log_message('error', '[FILE: ' . __file__ . ']  [public function: ' . __function__ . ']  [LINE: ' . __line__ . "]  [MESSAGE: deleting task(s) failed (delete_by: $delete_by is invalid)]");
            return false;
        }
    ...
    // -- tablecleanup ----------------------------------------------------------------------------------------------
    /**
     * This public function will do some clean up of the table:
    ...
    public function countTasks($id = '', $id_reference = 'project', $status = 'all', $show = 'all')
    {
    ...
    public function allMyTasksCounts($id = 0, $project_id = '')
    {
    ...
    public function searchTasks($offset = 0, $type = 'search', $members = 'all', $project_status='')
    {
        ...
        if ($this->input->get('tasks_status')) {
            ...
            $conditional_sql .= " AND tasks.tasks_status != 'completed'";
        }
        ...
        if ($this->input->get('memo') == 'not null') {
            $conditional_sql .= " AND tasks.memo!=''";
        }
        
        //added by Tomasz
        if($project_status == '')
        {
            $conditional_sql .= " AND projects.projects_status != 'hold'";    
        }
        else
        {
            $conditional_sql .= " AND projects.projects_status = '$project_status'";    
        }
        //end by Tomasz
        ...
        if ($members == 'mytasks') {
            $conditional_sql .= "
                AND (
                    tasks.tasks_assigned_to_id = ".$this->data['vars']['my_id']." OR
                    EXISTS (select 1 from tasks_viewers tv where tv.task_id=tasks.tasks_id and tv.team_profile_id=".$this->data['vars']['my_id'].")
                )
            ";
        }
    ...
    public function getTasksByProjectID($id = 0)
    {
    ...
    public function myPendingTasks($limit = 0)
    {
        ...
        //_____SQL QUERY_______
        $query = $this->db->query("SELECT tasks.*, projects.*, tasks_stage.*
                                          FROM tasks
                                            LEFT OUTER JOIN projects
                                            ON projects.projects_id = tasks.tasks_project_id
                                            LEFT JOIN tasks_stage
                                            ON tasks_stage.tasks_stage_id = tasks.accepted
                                          WHERE tasks.tasks_assigned_to_id = $my_id
                                                AND tasks.tasks_status NOT IN ('completed')
                                          $limit_sql");
...
// -- editDescription ----------------------------------------------------------------------------------------------
    ...
    public function updateStage($task_id, $accepted, $hourslogged)
    {
        $data = array(
            'accepted'           => $accepted,
            'hourslogged_manual' => $hourslogged
        );
        $this->db->where('tasks_id', $task_id);
    ...
    public function updateDescription($task_id, $tasks_description)
    {
        $data = array(
    ...
    * get list of time doc tasks
    */
    public function getListTimeDocTask()
    {
        //profiling::
        $this->debug_methods_trail[] = __function__;
        ...
        $query = $this->db->query("
            SELECT
                ...
                t.timedoctortaskid,
                i.team_profile_id,
                i.timedoctor_user_id
            FROM tasks t, timedoctor_tokens i
            WHERE t.timedoctortaskid > 0
                  AND tasks_status != 'completed'
                  AND i.team_profile_id = t.tasks_assigned_to_id
        ");
        $results = $query->result_array();
    ...
    public function updateHoursLogged($task_id, $hourslogged)
    {
        $data = array(
            'hourslogged' => (float)$hourslogged
        );
        $this->db->where('tasks_id', $task_id);
        $this->db->update('tasks', $data);
    ...
    public function setTimeDocTask($task_id, $timedoctortaskid)
    {
        //profiling::
        $this->debug_methods_trail[] = __function__;
    ...
    public function taskofferTask($task_id)
    {
        //profiling::
        $this->debug_methods_trail[] = __function__;
        ...
        //escape all post item
        $taskoffer = intval($this->input->post('taskoffer'));
        $feedback = $this->db->escape($this->input->post('feedback'));

        //----------sql & benchmarking start----------
        $this->benchmark->mark('code_start');

        if($taskoffer == 1) {
            $conditional_sql = "UPDATE tasks SET accepted = 1 WHERE tasks_id = $task_id";
        }
        if($taskoffer == 2) {
            $conditional_sql = "UPDATE tasks SET accepted = 3, taskoffer_feedback = $feedback WHERE tasks_id = $task_id";
        }
    ...
    public function editAssigned()
    {
        ...
        */
        $newTask = $this->db->escape('"'.$tasksData->tasks_text.' (reassigned)"');
    ...
    // -- editAssignedTime ----------------------------------------------------------------------------------------------
    /**
     * change user assigned time to a task
     *
     * 
     * @param   void
     * @return  numeric [affected rows]
    ...
    public function editAssignedTime($tasks_id, $assigned_time)
    {
        //profiling::
        $this->debug_methods_trail[] = __function__;
        ...
        //escape all post item
        $tasks_id = intval($tasks_id);
        $assigned_time = floatval($assigned_time);
        ...
        $query = $this->db->query("UPDATE tasks
                                          SET 
                                          tasks_text = CONCAT(tasks_text, ' (change assignes time)'),
                                          estimatedhours = $assigned_time
                                          WHERE tasks_id = $tasks_id");
    ...
    public function getTaskAssignedUser($task_id = '')
    {
    ...
    public function getSubTasks($task_id, $projectid){
      $task_id = $this->db->escape($task_id);
    ...
    public function getTimer($task_id, $projectid, $userid)
    {
      //profiling::
        $this->debug_methods_trail[] = __function__;
    ...
    public function superUsers($task_id = '')
    {
    ...
    public function bulkDelete($projects_list = '')
    {
        ...
        for ($i = 0; $i < count($lists); $i++) {
            if (! is_numeric(trim($lists[$i]))) {
                //log error
                log_message('error', '[FILE: ' . __file__ . ']  [public function: ' . __function__ . ']  [LINE: ' . __line__ . "]  [MESSAGE: Bulk Deleting tasks, for projects($clients_projects) failed. Invalid projects list]");
                //exit
                return false;
            }
        }
    ...
    public function listStages()
    {
        ...
        $this->db->select('*');
        $this->db->from('tasks_stage');
        $this->db->order_by('tasks_stage_order');
        $query = $this->db->get();
